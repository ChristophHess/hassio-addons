...
sudo docker build --build-arg BUILD_FROM="homeassistant/amd64-base:latest" -t local/octoprint_test .
sudo docker run -it --rm -v $(pwd)/my_test_data:/data -v $(pwd)/my_test_config:/config --net=host -p 5000:5000 --device=/dev/video0 --entrypoint /bin/sh local/octoprint_test
